( function( factory ) {
  ...
  Calendar.prototype.setEvents = function() {
    ...
    $(this.events).each(function(index, event) {
      that.processingEvents.call(that, event);
    });
  };

  Calendar.prototype.processingEvents = function(event) {
    var that = this;
    var dateArray = [];
    var mtitle = title;
    ...
    if(processing.length != 0) {
      ...
      var exclude = processing[0].exclude || [];
      dateArray = this.getDates(start, end, exclude);
    }
    ...
    for (var i = 0; i < dateArray.length; i ++ ) {
      ...
      // var mcalendarCellType = (date < this.timeCurrent) ? ('calendar-cell-type-past ' + calendarCellType) : calendarCellType;
      ...
      $(that.ui.parents).find(eventSelector).append('<div class="event ' + eventType + '" data-priority=' + priority + ' data-type=' + event.gType + '>' + mtitle + '</div>');
      var maxPriority = 0;
      var classPriority = '';
      $(that.ui.parents).find(eventSelector).find('.event').each(function(){
        var priority = parseInt($(this).data('priority'));
        maxPriority =  priority > maxPriority ? priority : maxPriority;
        if(priority == maxPriority) {
          if($(this).data('type') == 'general') {
            classPriority = (date < that.timeCurrent) ? 'calendar-cell-type-past' : calendarCellType;
          } else {
            classPriority = "calendar-cell-type-" + $(this).data('type');            
          }
        }
      });
      $(that.ui.parents).find(calendarCellSelector).addClass(classPriority);
      var numberEvents = $(that.ui.parents).find(eventSelector).find('.event').length;
      if(numberEvents > 1) {
        $(that.ui.parents).find(calendarCellSelector).append('<div class="events-number"><div class="inner-number">'+ numberEvents +'</div></div>');
        $(that.ui.parents).find(eventSelector).hide();
      }
      mtitle = '&nbsp;';
    }
  }

  Calendar.prototype.getDates = function(startDate, stopDate, exclude) {
    var dateArray = [];
    var currentDate = startDate;
    var excludeDate = $.map(exclude, function(item) {
    ...
    while (currentDate <= stopDate) {
      if(currentDate.getDay() != 0 && currentDate.getDay() != 6) {
        ...
        var accepted = true;
        $.each(excludeDate, function(index, date){
          if(date.getDate() == mdate.getDate() && date.getMonth() == mdate.getMonth() && date.getFullYear() == mdate.getFullYear()) {
            accepted = false;
            return false;
          }
  ...
  Calendar.prototype.show = function(month, year) {
    ...
    $(that.ui.parents).find(that.ui.events).removeClass('calendar-cell-type-general calendar-cell-type-holiday calendar-cell-type-past');
    $(that.ui.parents).find('.events-number').remove();
    var pre = 1;
